...
## 6. 위치 데이터 렌더링 흐름도
![Flow](docs/flow.png)
- 장소 목록 조회 : React에서 Spring Boot REST API로 장소 목록 데이터 요청

- 지도 마커 표시 : 응답 데이터의 위도·경도 값을 기준으로 Google Maps 위에 마커를 표시

- 상세 정보 표시 : 마커 클릭 시 선택된 장소 정보를 상태로 관리하고, 상세 카드에 이름, 주소, 카테고리, 설명, 이미지 표시
